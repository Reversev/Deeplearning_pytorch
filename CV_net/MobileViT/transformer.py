...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x_q: Tensor) -> Tensor:
        ...
        # self-attention
        # [N, P, C] -> [N, P, 3C] -> [N, P, 3, head, c] where C = head*c
        qkv = self.qkv_proj(x_q).reshape(b_sz, n_patches, 3, self.num_heads, -1)

        # [N, P, 3, head, c] -> [N, head, 3, P, c]
        qkv = qkv.transpose(1, 3).contiguous()

        # [N, head, 3, P, c] -> [N, head, P, c] * 3
        query, key, value = qkv[:, :, 0], qkv[:, :, 1], qkv[:, :, 2]

        query = query * self.scaling

        # [N, head, P, c] -> [N, head, c, P]
        key = key.transpose(-1, -2)

        # QK^T
        # [N, head, P, c] x [N, head, c, P] -> [N, head, P, P]
        attn = torch.matmul(query, key)
        attn = self.softmax(attn)
        attn = self.attn_dropout(attn)

        # weighted sum
        # [N, head, P, P] x [N, head, P, c] -> [N, head, P, c]
        out = torch.matmul(attn, value)

        # [N, head, P, c] -> [N, P, head, c] -> [N, P, C]
