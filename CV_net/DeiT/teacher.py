...
def parse_option():
    parser = argparse.ArgumentParser("Pytorch Training for MobileViT")
    parser.add_argument('--data', default="flowers", type=str, help='path to dataset')
    parser.add_argument('--teacher-model', default='resnet34', help='model name in timm.models')
...
def main_worker(gpu, ngpus_per_node, log, args):
    ...
    model = create_model(model_name=args.teacher_model,
                         pretrained=True,
        ...
        if best_acc1 < val_acc1:
            ...
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                ...
                torch.save({'epoch': epoch + 1,
                           ...
                           './checkpoints/teacher_{}_seed{}_best.pt'.format(args.teacher_model, str(args.seed)))
        else:
            pass
...
def main():
    ...
    args.model_name = '{}_{}_seed{}'.format(args.teacher_model, args.data, str(args.seed))
    make_dir('./logs/')
    if args.evaluate:
        log = Logger(args=args, filename='./logs/' + args.model_name + "_teacher_test.txt")
    else:
        log = Logger(args=args, filename='./logs/' + args.model_name + "_teacher.txt")
    print(args)
