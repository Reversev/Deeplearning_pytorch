...
from WideResNet_cifar import wrn_cifar, wrn_40_2, wrn_50_2, wrn_101_2, wrn_28_10


def argspar():
    parser = argparse.ArgumentParser(description='WRN PyTorch Training')
    parser.add_argument('--dataset-name', '-d', default='cifar10')
    ...
    parser.add_argument('--model-name', type=str, default='wrn_22_10',
                        help='model name: wrn_28_10, wrn_40_2, wrn_50_2, wrn_101_2')
    ...
    parser.add_argument('--load_model', default='checkpoint_139.pt', type=str,
...
def main():
    ...
    image_path = "../../datasets/" + args.dataset_name + "/"
    ...
    if 'cifar100' in args.dataset_name:
        train_dataset = datasets.CIFAR100(root=image_path, train=True,
                                          download=True, transform=data_transform['train'])

        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size,
                                                   shuffle=True, num_workers=net_count)

        val_dataset = datasets.CIFAR100(root=image_path, train=False,
        ...
        val_dataset = datasets.CIFAR10(root=image_path, train=False,
                                       download=True, transform=data_transform['val'])
    ...
    depth_wide = args.model_name[4:].split("_")
    net = wrn_cifar(num_classes=CLASS_NUM,
                    depth=int(depth_wide[0]),
                    widen_factor=int(depth_wide[1]), dropRate=args.drop_rate)

    start_epoch = args.start_epoch
    net = net.to(device)
    logger.write('Total params: {:.2f}M \n'.format(sum(p.numel() for p in net.parameters()) / (1000 * 1000)))
    loss_function = nn.CrossEntropyLoss()
    optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-04, nesterov=True)
    best_acc = 0.0
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if top1.avg > best_acc:
            ...
            }, filename=os.path.join(args.checkpoint, 'checkpoint_' + args.model_name + str(epoch + 1) + '.pt'))
