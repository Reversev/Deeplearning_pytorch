...
# create model
net = GoogLeNet(n_classes=CLASS_NUM, aux_logits=False)
net = net.to(device)

# load model weights
assert os.path.exists(MODEL_PATH), "file: '{}' does not exist.".format(MODEL_PATH)
missing_keys, unexpected_keys = net.load_state_dict(torch.load(MODEL_PATH), state_dict=False)
